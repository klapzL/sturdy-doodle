...
async def get_event(db: DBSession, event_id: int):
    return await EventService.get_obj(db, [EventService.model.id == event_id])
...
async def update_evente(db: DBSession, event_id: int, event: EventCreate):
    await EventService.update(
        db, [EventService.model.id == event_id], event.model_dump()
    )
...
async def remove_event(db: DBSession, event_id: int):
    await EventService.delete(db, [EventService.model.id == event_id])
